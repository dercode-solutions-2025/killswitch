...
Another thing, the hackers have to guess the name AND extension. KILLSWITCH's files are also encrypted with an AES encryption system created by the United States government, you know them, only the most SECURE, PRIVATE, POWERFUL country to exist since 1776.
AES was created in 2001, developed in 1998, but don't undermine it. It is used by finance agenices, government agencies (to protect files, that when leaked, compromise the entire infrastructure of the United States, and so they must be safe.), AES is also used by telecom agencies.
AES is a similar system to our system, unreadable files, blah blah blah, yachta yachta yachta.
KILLSWITCH also sports a nice ASCII banner and kill count. It also has every possible malware name, in the dozens.

See WARNING.txt for more information regarding user liability.
